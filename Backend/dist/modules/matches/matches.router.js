...
import { balancePadelTeams, inferWinnerTeam, isDoublesStyle, teamsPartitionPlayers, } from "../../lib/matchTeams.js";
...
matchesRouter.post("/:id/confirm", async (req, res) => {
    ...
    if (match.confirmedPlayerEmails.includes(email))
        return res.json(match);
    const updated = await prisma.match.update({
        where: { id: req.params.id },
...
matchesRouter.post("/:id/start", async (req, res) => {
    ...
    if (match.players.length === 2 && (teamA.length === 0 || teamB.length === 0)) {
        teamA = [match.players[0]];
        teamB = [match.players[1]];
    }
    if (isDoublesStyle(match) && match.players.length >= 4) {
        if (!match.teamsLocked) {
            return res.status(409).json({ error: "Organiser must lock teams before starting doubles" });
        }
        if (!teamsPartitionPlayers(teamA, teamB, match.players)) {
            return res.status(400).json({ error: "Teams must partition all players" });
        }
        if (teamA.length !== 2 || teamB.length !== 2) {
            return res.status(400).json({ error: "Doubles requires two players on each team" });
        }
    }
    const updated = await prisma.match.update({
        where: { id: req.params.id },
...
matchesRouter.post("/:id/submit-score", async (req, res) => {
    const { scoreTeamA, scoreTeamB, winnerTeam, submittedBy, email, } = req.body;
...
matchesRouter.post("/:id/confirm-score", async (req, res) => {
    ...
    const w = match.pendingWinnerTeam ||
