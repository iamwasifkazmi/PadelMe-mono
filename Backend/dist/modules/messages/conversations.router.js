...
function normalizeParticipantList(emails) {
    const seen = new Set();
    const out = [];
...
function uniqueNormParticipants(participantEmails) {
    return [...new Set(participantEmails.map(normEmail).filter(Boolean))];
}
function isDirectPair(type, participantEmails) {
    return type === "direct" && uniqueNormParticipants(participantEmails).length === 2;
}
async function areAcceptedFriends(emailA, emailB) {
...
conversationsRouter.get("/:id/messages", async (req, res) => {
    ...
    if (isDirectPair(conversation.type, conversation.participantEmails)) {
        ...
        if (!policy.ok)
            return res.status(403).json({ error: policy.error });
    }
...
conversationsRouter.post("/:id/messages", async (req, res) => {
    ...
    for (const email of conversation.participantEmails) {
        if (normEmail(email) === normEmail(senderEmail))
            continue;
    ...
    for (const rawEmail of conversation.participantEmails) {
        if (normEmail(rawEmail) === normEmail(senderEmail))
            continue;
        await notifyUser({
