...
import { CompetitionFormat, CompetitionStatus, CompetitionType, } from "@prisma/client";
import { prisma } from "../../lib/prisma.js";
import { advanceBracketAfterConfirmedMatch, generateKnockoutRound1, resolveParticipantSlots, } from "../../lib/competitionBracket.js";
import { generateLeagueWeekFixtures } from "../../lib/competitionLeague.js";
import { confirmCompetitionMatchScore, submitCompetitionMatchScore, validateSubmittedCompetitionScore, } from "../../lib/competitionMatchScore.js";
...
competitionsRouter.post("/:id/join", async (req, res) => {
    ...
    if (!competition)
        return res.status(404).json({ error: "Competition not found" });
    if (competition.participants.includes(email))
        return res.json(competition);
...
competitionsRouter.post("/:id/start", async (req, res) => {
    ...
    if (competition.type === CompetitionType.league) {
        const fixtures = await generateLeagueWeekFixtures({ ...competition, status: CompetitionStatus.in_progress }, 1);
    ...
    return res.json({
        status: "started",
        message: competition.type === "league"
...
competitionsRouter.post("/:id/advance-bracket", async (req, res) => {
    ...
    if (!competition)
        return res.status(404).json({ error: "Competition not found" });
    const confirmedMatchId = String(req.body.confirmedMatchId || req.body.confirmed_match_id || "");
    if (!confirmedMatchId) {
        return res.status(400).json({ error: "confirmedMatchId is required" });
    }
    const result = await advanceBracketAfterConfirmedMatch(competition.id, confirmedMatchId);
...
competitionsRouter.post("/:id/matches/:matchId/submit-score", async (req, res) => {
    ...
    try {
        const updated = await submitCompetitionMatchScore(competition, match, email, scoreP1, scoreP2);
        return res.json(updated);
    }
    catch (e) {
...
competitionsRouter.post("/:id/matches/:matchId/confirm-score", async (req, res) => {
    ...
    if (!match)
        return res.status(404).json({ error: "Match not found" });
    try {
        const updated = await validateSubmittedCompetitionScore(competition, match, email, true);
        return res.json(updated);
    }
    catch (e) {
        return res.status(400).json({ error: e.message });
...
competitionsRouter.post("/:id/matches/:matchId/reject-score", async (req, res) => {
    ...
    if (!match)
        return res.status(404).json({ error: "Match not found" });
    try {
        const updated = await validateSubmittedCompetitionScore(competition, match, email, false, reason);
        return res.json(updated);
    }
    catch (e) {
        return res.status(400).json({ error: e.message });
...
competitionsRouter.post("/:id/matches/:matchId/host-score", async (req, res) => {
    ...
    if (!match)
        return res.status(404).json({ error: "Match not found" });
    try {
        const updated = await confirmCompetitionMatchScore(competition, match, email, scoreP1, scoreP2, { hostOverride: true });
        return res.json(updated);
    }
    catch (e) {
        return res.status(400).json({ error: e.message });
