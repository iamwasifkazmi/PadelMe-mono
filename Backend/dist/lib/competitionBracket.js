...
export async function resolveParticipantSlots(emails) {
...
function displayDoublesTeam(members) {
    return members.map((m) => m.name).join(" & ");
}
/** Generate round-1 knockout matches (Base44 parity). */
export async function generateKnockoutRound1(competition, slots) {
    const isDoubles = competition.teamStructure === "doubles" ||
...
export async function advanceBracketAfterConfirmedMatch(competitionId, confirmedMatchId) {
    ...
    const winners = currentRoundMatches.map((m) => {
        ...
        if (isDoubles) {
            const winEmails = m.winnerTeam === "team_a" ? m.teamAEmails || [] : m.teamBEmails || [];
        ...
        const winName = winEmail === m.player1Email ? m.player1Name : m.player2Name;
        return {
            email: winEmail,
