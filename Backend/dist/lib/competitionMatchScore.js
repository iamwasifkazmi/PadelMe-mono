...
function matchParticipantEmails(m) {
    if (isDoublesMatch(m)) {
        return [...(m.teamAEmails || []), ...(m.teamBEmails || [])];
    }
    return [m.player1Email, m.player2Email].filter(Boolean);
}
function resolveWinner(m, s1, s2) {
    if (isDoublesMatch(m)) {
        if (s1 === s2)
            return { winnerEmail: null, winnerTeam: null };
...
export async function confirmCompetitionMatchScore(competition, match, actorEmail, scoreP1, scoreP2, opts) {
...
export async function submitCompetitionMatchScore(competition, match, actorEmail, scoreP1, scoreP2) {
...
export async function validateSubmittedCompetitionScore(competition, match, actorEmail, accept, rejectionReason) {
    const participants = matchParticipantEmails(match).map((e) => e.toLowerCase());
    if (!participants.includes(actorEmail.toLowerCase())) {
    ...
    return confirmCompetitionMatchScore(competition, match, actorEmail, match.submittedScoreP1 || "", match.submittedScoreP2 || "");
}
