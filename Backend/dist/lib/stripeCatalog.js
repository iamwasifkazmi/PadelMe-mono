...
export async function getStripeBillingIdsFromDb() {
...
export async function saveStripeBillingIds(ids) {
...
async function findOrCreateProduct(stripe, existingProductId) {
    ...
    const match = listed.data.find((p) => p.metadata?.mipadel_product === PRODUCT_META.mipadel_product || p.name === PRODUCT_NAME);
    if (match)
        return match.id;
...
async function findOrCreatePrice(stripe, productId, existingPriceId) {
    if (existingPriceId) {
        try {
    ...
    const monthlyGbp = prices.data.find((p) => p.currency === PRICE_CURRENCY &&
 ...
 */
export async function ensureStripeBillingCatalog() {
