...
export function kFactorFromMatchTags(tags) {
...
export function skillLabelFromElo(elo) {
    if (elo >= 1600)
        return "elite";
    if (elo >= 1400)
        return "advanced";
...
export function inactivityDecayAmount(storedElo, lastRatedMatchAt, now = new Date()) {
    if (!lastRatedMatchAt)
        return 0;
...
export function resolveEffectiveElo(storedElo, lastRatedMatchAt, now = new Date()) {
    const pull = inactivityDecayAmount(storedElo, lastRatedMatchAt, now);
    if (pull === 0)
        return clampElo(storedElo);
    if (storedElo === DEFAULT_ELO)
        return DEFAULT_ELO;
    const direction = storedElo > DEFAULT_ELO ? -1 : 1;
    return clampElo(storedElo + direction * pull);
}
export function averageElo(values) {
    if (!values.length)
        return DEFAULT_ELO;
    return values.reduce((a, b) => a + b, 0) / values.length;
}
export function eloDelta(playerEffective, opponentAvgEffective, actualScore, k) {
