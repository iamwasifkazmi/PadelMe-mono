const LTA_BASE = process.env.LTA_PADEL_FINDER_URL?.trim() ||
    "https://www.ltapadel.org.uk/play/find-a-padel-court/";
const LTA_UA = process.env.LTA_USER_AGENT ||
...
export function ltaCourtFinderEnabled() {
...
export function parseLtaCourtFinderHtml(html) {
    ...
    for (let i = 1; i < chunks.length; i++) {
        ...
        let name = "";
        const spanTitle = block.match(/<h2[^>]*class="lta-card-title[^"]*"[^>]*>[\s\S]*?<span>([^<]+)<\/span>/i);
        if (spanTitle?.[1])
            name = decodeHtml(spanTitle[1]);
        if (!name) {
            const linkTitle = block.match(/<h2[^>]*class="lta-card-title[^>]*>[\s\S]*?<a[^>]*>([^<]+)<\/a>/i);
            if (linkTitle?.[1])
                name = decodeHtml(linkTitle[1]);
        }
        if (!name)
            continue;
        ...
        if (seen.has(key))
            continue;
        seen.add(key);
        const detail = block.match(/([A-Z]{1,2}\d{1,2}[A-Z]?\s*\d[A-Z]{2})\s*\(([0-9.]+)\s*miles away\)/i);
        const postcode = detail?.[1]?.trim();
        const distanceMiles = detail?.[2] ? Number.parseFloat(detail[2]) : undefined;
        let bookingUrl;
        const siteLink = block.match(/<a[^>]+href="(https?:\/\/[^"]+)"[^>]*target="_blank"[^>]*>[\s\S]*?View court details/i);
        ...
        const addressParts = [postcode, distanceMiles != null ? `${distanceMiles} mi` : null].filter(Boolean);
        out.push({
            name,
...
export async function searchLtaPadelCourts(query) {
    if (!ltaCourtFinderEnabled())
        return [];
    const center = await geocodeQuery(query.includes("United Kingdom") ? query : `${query}, UK`);
    if (!center)
        return [];
    const searchUrl = `${LTA_BASE.replace(/\/$/, "")}/?latitude=${encodeURIComponent(center.lat)}` +
    ...
    return parsed.slice(0, 25).map((row) => {
        const address = row.postcode && row.distanceMiles != null
