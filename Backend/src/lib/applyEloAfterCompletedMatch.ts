...
import {
  ...
  averageElo,
  eloDelta,
  kFactorFromMatchTags,
  resolveEffectiveElo,
  skillLabelFromElo,
} from "./elo.js";

function normalizeWinner(w: string | null | undefined): "team_a" | "team_b" | null {
  if (!w) return null;
  const s = String(w).toLowerCase().replace(/[\s-]+/g, "_");
  if (s === "team_a" || s === "teama") return "team_a";
  if (s === "team_b" || s === "teamb") return "team_b";
  return null;
}
 ...
 */
export async function applyEloAfterCompletedMatch(match: Match): Promise<void> {
  ...
  const oppAvgFor = (email: string): number => {
  ...
  const calcs: ParticipantCalc[] = allRated.map((email) => {
    ...
    const won =
      (winner === "team_a" && onA) || (winner === "team_b" && !onA);
  ...
  const scoreSummary =
    [match.scoreTeamA, match.scoreTeamB].filter(Boolean).join(" · ") || null;
