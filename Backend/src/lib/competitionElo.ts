...
export async function applyEloAfterCompetitionMatch(
  ...
  const isDoubles =
    (match.teamAEmails?.length ?? 0) > 0 || (match.teamBEmails?.length ?? 0) > 0;
  ...
  if (isDoubles) {
    ...
    winners = [w];
    const loser =
      w === match.player1Email ? match.player2Email : match.player1Email;
    if (loser) losers = [loser];
  }
  ...
  const resolveEmail = (raw: string) => byLower.get(raw.trim().toLowerCase())?.email;
  const winnerCanon = winners.map(resolveEmail).filter(Boolean) as string[];
  const loserCanon = losers.map(resolveEmail).filter(Boolean) as string[];
  if (winnerCanon.length !== winners.length || loserCanon.length !== losers.length) return;

  const winnerAvg = averageElo(
    winnerCanon.map((e) => {
      const u = byLower.get(e.toLowerCase())!;
      return resolveEffectiveElo(u.eloRating, u.playerStats?.lastMatchAt ?? null, now);
    }),
  );
  const loserAvg = averageElo(
    loserCanon.map((e) => {
      const u = byLower.get(e.toLowerCase())!;
      return resolveEffectiveElo(u.eloRating, u.playerStats?.lastMatchAt ?? null, now);
    }),
  ...
  for (const email of winnerCanon) {
    ...
    const effective = resolveEffectiveElo(
      u.eloRating,
      u.playerStats?.lastMatchAt ?? null,
      now,
    );
