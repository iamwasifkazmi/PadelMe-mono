...
async function canonicalEmail(raw: string): Promise<string> {
...
async function userNotifyPrefs(email: string): Promise<{
  email: string;
...
export async function notifyInstantPlayMatched(opts: {
  ...
  await Promise.all(
    opts.recipientEmails.map(async (raw) => {
      const prefs = await userNotifyPrefs(raw);
      if (!prefs.notifyInstantPlay) return;
      if (opts.excludeEmail && emailsEqual(prefs.email, opts.excludeEmail)) return;
...
export async function notifyInstantPlayPlayerJoined(opts: {
  ...
  const seen = new Set<string>();
  await Promise.all(
    opts.rosterEmails.map(async (raw) => {
      const prefs = await userNotifyPrefs(raw);
      if (!prefs.notifyInstantPlay) return;
      if (emailsEqual(prefs.email, opts.joinerEmail)) return;
      const key = prefs.email.toLowerCase();
      if (seen.has(key)) return;
      seen.add(key);
      await notifyUser({
...
export async function notifyNearbyInstantPlaySeekers(opts: {
  ...
  const place = (opts.locationName || "").trim() || "nearby";
  const formatLabel =
    opts.matchType === MatchType.singles
      ? "singles"
  ...
  const skip = new Set(
    (opts.alreadyNotified || []).map((e) => e.trim().toLowerCase()).filter(Boolean),
  );
