...
export async function syncPlayerRatingSummary(ratedEmail: string) {
  ...
  for (const key of TAG_KEYS) tagCounts[key] = 0;
  for (const r of ratings) {
    for (const tag of r.tags) {
      const norm = String(tag).trim().toLowerCase();
      if (TAG_KEYS.includes(norm as (typeof TAG_KEYS)[number])) {
