...
function shouldSendUserToOnboarding(user: Parameters<typeof userNeedsOnboarding>[0]): boolean {
...
function appleClientId(): string {
  for (const key of ["APPLE_CLIENT_ID", "APPLE_BUNDLE_ID", "IOS_BUNDLE_ID"] as const) {
...
authRouter.post("/google", async (req, res) => {
  ...
  const fullNameFromGoogle =
    typeof payload.name === "string" && payload.name.trim().length >= 2 ? payload.name.trim() : null;
  ...
  const user = existing
    ? await prisma.user.update({
        ...
        data: {
          ...
          fullName:
            existing.fullName && existing.fullName.trim().length >= 2
              ? existing.fullName
...
authRouter.post("/apple", async (req, res) => {
  ...
  if (!appleAudience) {
    return res.status(503).json({
      error:
        "Apple Sign In is not configured (set APPLE_CLIENT_ID to your iOS bundle ID, e.g. com.mipadel)",
    });
  }
  ...
  let applePayload: jwt.JwtPayload;
  try {
  ...
  const emailFromToken =
    typeof applePayload.email === "string" && applePayload.email.trim().length > 0
  ...
  const fullNameFromToken =
    typeof applePayload.name === "string" && applePayload.name.trim().length >= 2 ? applePayload.name.trim() : null;
  const fullNameResolved =
    fullNameFromClient && fullNameFromClient.trim().length >= 2
  ...
  if (!existing) {
    if (!email) {
      return res.status(400).json({
        error:
          "Apple did not return an email for this sign-in. Try again, or remove the app from Apple ID settings and sign in once more to share email.",
      });
    }
